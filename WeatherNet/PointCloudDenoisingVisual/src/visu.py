...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description='Visualize point cloud HDF5 (e.g. inference output from test.py)')
    p.add_argument('--path', type=str, default=PATH,
                    help='directory containing *.hdf5 files (default: {} )'.format(PATH))
    p.add_argument('--max-frames', type=int, default=MAX_FRAMES, help='max frames to publish')
    args = p.parse_args()
    path = args.path.rstrip('/') + '/'
    main(path=path, max_frames=args.max_frames)
