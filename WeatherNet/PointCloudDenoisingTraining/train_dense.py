...
from weathernet.datasets import DENSE, WADS, Normalize, Compose, RandomHorizontalFlip
...
def get_data_loaders(data_dir, batch_size, val_batch_size, num_workers, dataset_cls):
    """dataset_cls: DENSE or WADS (defines classes, label_mapping, mean, std)."""
    normalize = Normalize(mean=dataset_cls.mean(), std=dataset_cls.std())
    transforms = Compose([
    ...
    seq_split = getattr(dataset_cls, 'VAL_SEQUENCES', None)  # WADS: train/val by sequence, no train_01/val_01
    train_loader = DataLoader(
        dataset_cls(root=data_dir, split='train', transform=transforms, sequence_split_val_seqs=seq_split),
        batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
    val_loader = DataLoader(
        dataset_cls(root=data_dir, split='val', transform=val_transforms, sequence_split_val_seqs=seq_split),
...
def run(args):
    ...
    dataset_cls = WADS if getattr(args, 'label_map', None) == 'wads' else DENSE
    ...
    train_loader, val_loader = get_data_loaders(args.dataset_dir, args.batch_size, args.val_batch_size,
                                                args.num_workers, dataset_cls=dataset_cls)

    criterion = nn.CrossEntropyLoss(weight=dataset_cls.class_weights()).to(device)
    ...
    def _prepare_batch(batch, non_blocking=True):
        # Dataset may return extra metadata (e.g., sensorX/Y/Z). Keep training
        # compatible by ignoring any additional returned values.
        distance, reflectivity, target, *_ = batch
    ...
    def run_validation(engine):
        ...
        iou_text = ', '.join(['{}: {:.1f}'.format(dataset_cls.classes[i + 1].name, v) for i, v in enumerate(iou.tolist())])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--label-map", type=str, default=None, choices=['wads'],
                        help="remap labels in dataloader (e.g. wads: 110,111->snow 101, others->clear 100)")
