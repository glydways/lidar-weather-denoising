...
class Fill2d(nn.Module):
    ...
    def forward(self, x, idx_valid):
        ...
        denom = idx_y.sum(dim=3).clamp(min=1e-8)

        mu = (y * idx_y).sum(dim=3) / denom
        mu = mu.reshape(n, c, h, 1).tile(1, 1, 1, w)
        sigma2 = ((y - mu).pow(2) * idx_y).sum(dim=3) / denom
        sigma2 = sigma2.clamp(min=0).reshape(n, c, h, 1).tile(1, 1, 1, w)
