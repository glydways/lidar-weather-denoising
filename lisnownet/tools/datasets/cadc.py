...
class CADC(Base):
    ...
    def read_file_list(self, data_dir, train_ratio=0.8):
        ...
        # Labeled devkit layout: date/drive/labeled/lidar_points/data/*.bin
        # Raw layout: date/drive/raw/lidar_points_corrected/data/*.bin
        lidar_globs = [
            'labeled/lidar_points/data/*.bin',
            'raw/lidar_points_corrected/data/*.bin',
        ]
        fn_points = []
        for f in fn_annotations:
            for p in lidar_globs:
                found = sorted(glob(f.replace('3d_ann.json', p)))
                if found:
                    fn_points += found
                    break
        return fn_points
