...
parser.add_argument('--data_dir', type=str, default=None,
                    help='Root directory for datasets (contains cadcd/ and wads/). Overrides --cadc_dir and --wads_dir if set.')
parser.add_argument('--cadc_dir', type=str, default='./data/cadcd',
                    help='Path to CADC dataset root (default: ./data/cadcd). Ignored if --data_dir is set.')
parser.add_argument('--wads_dir', type=str, default='./data/wads',
                    help='Path to WADS dataset root (default: ./data/wads). Ignored if --data_dir is set.')
parser.add_argument('--log_dir', type=str, default='./logs')
parser.add_argument('--tag', type=str, default='')
config = vars(parser.parse_args())
# Normalize data_dir -> cadc_dir / wads_dir (may be overwritten when loading config)
...
# Ensure dataset path args exist (e.g. when resuming from old config)
config.setdefault('data_dir', None)
config.setdefault('cadc_dir', './data/cadcd')
config.setdefault('wads_dir', './data/wads')
if config.get('data_dir') is not None:
    config['cadc_dir'] = os.path.join(config['data_dir'], 'cadcd')
    config['wads_dir'] = os.path.join(config['data_dir'], 'wads')
...
if config['dataset'] == 'cadc':
    ds_train = CADC(config['cadc_dir'], training=True)
    ds_val = CADC(config['cadc_dir'], training=False)
elif config['dataset'] == 'wads':
    ds_train = WADS(config['wads_dir'], training=True)
    ds_val = WADS(config['wads_dir'], training=False)
