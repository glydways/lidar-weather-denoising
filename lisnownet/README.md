...
**WADS**: The pretrained WADS checkpoint uses `alpha=5.5` (see eval command below). To train on WADS:

```bash
$ ./train.py --dataset wads --wads_dir /path/to/wads \
  --tag wads_alpha=5.5 --alpha 5.5 --batch_size 32 --num_epochs 20
```

Optional: `--data_dir /path/to/data` (expects `data/cadcd` and `data/wads`). Other defaults: `--lr 1e-3`, `--lr_decay 0.95`, `--beta 0.5`.
